...
description: Error and status-code conventions for keycloak-user-storage — ResponseStatusException, 204 on DELETE/PATCH, 409 on conflict, {"error": "..."} envelope. Use in /plan and /build.
---
# Error Handling

Ported from the former `.claude/conventions/01`.
...
- 현재 전역 `@ControllerAdvice` 없음 — `ResponseStatusException`은 Spring이 자동으로 JSON 에러 응답 생성. 전역 핸들러 도입 시 ADR로 기록.
- DELETE/PATCH 성공 시 본문 없이 **204** 반환 (200 + body 반환 금지)
- 에러 응답 포맷은 `{"error": "..."}` 로 통일 (BL-09 완료 — 전역 매핑)
