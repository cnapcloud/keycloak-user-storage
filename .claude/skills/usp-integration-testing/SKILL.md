---
name: usp-integration-testing
description: Integration test patterns for keycloak-user-storage — @SpringBootTest RANDOM_PORT, TestRestTemplate, PATCH support, raw-JSON null bodies, USP invariants. Use in /build red step and gap tests.
---
# Testing
...
```java
// Map.of()는 null 값 불허, NON_NULL 설정으로 직렬화 제외됨 → raw JSON String으로 전달
...
## Gradle 단일 테스트 실행
```bash
./gradlew test --tests 'com.keycloak.userstorage.UserStorageIntegrationTest.methodName'
```
