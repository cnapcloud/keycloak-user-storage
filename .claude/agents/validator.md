...
**Hat 선택:** 호출한 커맨드의 frontmatter `hat:` (`/validate` → `validate`, `/review` → `review`)를 따른다. 없으면 태스크 프롬프트의 `(validate hat)` / `(review hat)` 리터럴, 그다음 `## When invoked`의 커맨드명↔hat 매핑. 셋 다 불명확하면 사용자에게 어느 hat인지 묻는다.
