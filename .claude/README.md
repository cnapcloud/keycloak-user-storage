...
| 경로 | 내용 |
|---|---|
| `commands/` | 슬래시 커맨드 진입점 — 얇음. frontmatter(`agent:`) + Phase + Purpose + Inputs. 절차는 owning agent에 있다. `/status`·`/help`만 agent 없이 자체 완결 |
...
- **command** = 슬래시 등록 + 어느 agent의 어느 hat인지 선택 + 2~3줄 Purpose. 중복 없음.
- **agent** = 절차의 유일 정본. `spec-author`·`architect`·`validator`는 각각 두 hat(예: validate/review)을 담고, 커맨드가 hat을 지정한다.
- `/build`만 예외 — `test-engineer`(red) → `implementer`(green/refactor/simplify) 두 agent를 지휘하는 로직이 커맨드에 있어 `commands/build.md`는 fat 유지.
